...
public class Zoo_Monkey {
    public static int monkeyLeft(int n,int k,int j,int m,int p){
        int eatBanana=m/k;
        int eatPeanut=p/j;

        int total=eatPeanut+eatBanana;
        if(m%k !=0 || p%j !=0){
            total++;
        }
        return total;
    ...
    public static void main(String[] args) {
        ...
        System.out.println(monkeyLeft(n,k,j,m,p));
    }
}
/*
Q4. There are total n number of Monkeys sitting on the branches of a huge Tree. As travelers offer Bananas and Peanuts,
